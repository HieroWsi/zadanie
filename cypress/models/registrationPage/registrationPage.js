export default class RegistrationPage {
  ...
  typeEmailAdress(emailAdress) {
    cy.get(this.selectors.emailAdressInput)
      .should("be.visible")
      .type(emailAdress);
  }
  ...
  chooseFile(fileName) {
    ...
    cy.get(this.selectors.choosenFileInput).selectFile("@testFile.png", {
      force: true,
      action: "drag-drop",
    });
  }
  ...
  checkConditions(conditionIndex) {
    const conditionCheckbox = this.selectors.conditionCheckbox.replace(
      "<index>",
      conditionIndex
    );
